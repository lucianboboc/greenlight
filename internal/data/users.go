...
import (
	...
	"errors"
	"github.com/lib/pq"
	"github.com/lucianboboc/greenlight/internal/validator"
...
func (m UserModel) Insert(user *User) error {
	...
	if err != nil {
		...
		case errors.As(err, &pqError):
			if pqError.Code == "23505" && pqError.Constraint == "users_email_key" {
...
func (m UserModel) GetAllForPermission(permissionID int64) ([]User, error) {
	query := `
	SELECT users.id, users.created_at, users.name, users.email, users.password_hash, users.activated, users.version
	FROM users
	...
	rows, err := m.DB.QueryContext(ctx, query, permissionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
